...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    */
    ...
    if(req.userId === provider_id){
      return res.status(401).json({error: "You can't make appointments to yourself"})
    }
  ...
  async delete(req, res) {
    ...
    /**
     * O usuario só pode cancelar um agendamento até 2 horas antes do horario marcado,
     * após isso, não deve ser permitido ao usuario cancelar um agendamento.
     */
    const dateWhithSub = subHours(appointment.date, 2); 
    /**
     *   /\  
     *   ||  Este método (subHours) subtrai 2 horas do horario agendado para comparar com o horario atual.
     *   ||
     */
